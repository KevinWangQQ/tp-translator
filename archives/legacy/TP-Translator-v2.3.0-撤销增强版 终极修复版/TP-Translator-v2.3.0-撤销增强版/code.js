console.log("TP Translator v2.3.0 撤销增强版 loading...");
...
class TranslationEngine {
  ...
  async translateWithOpenAI(text, sourceLang, targetLang, config, glossary) {
    ...
    if (!model.includes('instruct')) {
      ...
      const translatedText = data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content && data.choices[0].message.content.trim();
  ...
  async translateWithGemini(text, sourceLang, targetLang, config, glossary) {
    ...
    const data = await response.json();
    let translatedText = data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0] && data.candidates[0].content.parts[0].text && data.candidates[0].content.parts[0].text.trim();
...
class MessageHandler {
  ...
  async handleMessage(message) {
    ...
    try {
      switch (message.type) {
        ...
        case 'undo-all-translations':
          return await this.handleUndoAllTranslations(message.payload);
          
        case 'batch-update-translations':
          return await this.handleBatchUpdateTranslations(message.payload);
  ...
  // 处理撤销所有翻译
  async handleUndoAllTranslations(payload) {
    try {
      ...
      console.log(`撤销 ${history.length} 个翻译节点`);
      ...
      for (const historyItem of history) {
        try {
          let node;
          try {
            node = figma.getNodeById(historyItem.nodeId);
          } catch (error) {
            node = figma.currentPage.findOne(n => n.id === historyItem.nodeId);
          }
          
          if (node && node.type === 'TEXT') {
            await figma.loadFontAsync(node.fontName);
            node.characters = historyItem.originalText;
            successCount++;
            console.log(`撤销节点 ${historyItem.nodeId}: "${historyItem.originalText}"`);
          } else {
            failureCount++;
            errors.push(`节点 ${historyItem.nodeId} 不存在或不是文本节点`);
          }
        } catch (error) {
          failureCount++;
          errors.push(`撤销节点 ${historyItem.nodeId} 失败: ${error.message}`);
          console.error(`撤销节点失败:`, error);
        }
      }
      ...
      return {
        ...
        payload: {
          ...
          failureCount,
          errors,
          message: `成功撤销 ${successCount} 个翻译${failureCount > 0 ? `, ${failureCount} 个失败` : ''}`
        }
      };
      
    } catch (error) {
      console.error('撤销翻译失败:', error);
      return {
        type: 'undo-result',
        payload: {
          success: false,
  ...
  // 处理批量更新翻译
  async handleBatchUpdateTranslations(payload) {
    try {
      const { originalText, newTranslation, nodeIds } = payload;
      
      console.log(`批量更新翻译: "${originalText}" → "${newTranslation}"，影响 ${nodeIds.length} 个节点`);
      ...
      for (const nodeId of nodeIds) {
        try {
          ...
          if (node && node.type === 'TEXT') {
            await figma.loadFontAsync(node.fontName);
            node.characters = newTranslation;
            successCount++;
            console.log(`更新节点 ${nodeId}: "${newTranslation}"`);
          } else {
            failureCount++;
            errors.push(`节点 ${nodeId} 不存在或不是文本节点`);
          }
        } catch (error) {
          failureCount++;
          errors.push(`更新节点 ${nodeId} 失败: ${error.message}`);
          console.error(`批量更新节点失败:`, error);
        }
      }
      
      return {
        type: 'batch-update-result',
        payload: {
          success: true,
          successCount,
          failureCount,
          errors,
          message: `成功更新 ${successCount} 个节点${failureCount > 0 ? `, ${failureCount} 个失败` : ''}`
        }
      };
...
class PluginManager {
  ...
  async initialize() {
    console.log('Initializing TP Translator v2.3.0 撤销增强版...');
    ...
    console.log('TP Translator v2.3.0 撤销增强版 initialized successfully');
  }
  ...
  setupUI() {
    figma.showUI(__html__, {
      width: 380,
      height: 600,
      title: 'TP Translator v2.3.0 撤销增强版'
    });
  }
...
console.log('TP Translator v2.3.0 撤销增强版 script loaded');